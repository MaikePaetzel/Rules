...
into the goal \added{while avoiding an obstacle}.
...
\item \added{The obstacle $O$ of diameter approximatively 0.5m is placed
    between the penalty mark and the ball,
    the projection of the obstacle on the ground should contain the midpoint
    between the ball and the penalty mark.}
...
\item Teams may start the robot $R_P$ manually by pressing a button when the
  ...
  \removed{
    If the pass is performed by a human, then the human is not allowed to wait before
    kicking.
    Once the whistle is blown the human has 2 seconds to kick the ball,
    otherwise the run is retaken.
  }
\item A chronometer is started when \added{the referee blows the whistle}
  \removed{$R_P$ or $H$ kicks the ball}.
\end{enumerate}
...
\item \textit{Failure}
  \begin{itemize}
    ...
    \item \added{The ball touches the obstacle $O$ before being touched by $R_k$.}
...
\item \textit{Retake}
  \begin{itemize}
    \item \added{$R_k$ or $R_P$ moves before the whistle is blown.}
    ...
    \item \removed{The pass is performed by a human and the human did not kick 2 seconds after
...
\item Average time for \textit{Success} runs, from \added{the moment the whistle
    was blown} \removed{first touch by $R_p$ or $H$} until goal is scored
